...
readings.forEach((reading, index) => {
    ...
    if (!isNaN(current1) && !isNaN(current2) && !isNaN(current3)) {
        ...
        intervalKey.setMinutes(intervalKey.getMinutes() >= 30 ? 60 : 30, 0, 0); // Rounds up to the nearest half hour
        intervalKey.setSeconds(0);
        intervalKey.setMilliseconds(0);

        // If the calculated interval is in the next hour, increment the hour
        if(intervalKey.getMinutes() === 60) {
            intervalKey.setHours(intervalKey.getHours() + 1);
            intervalKey.setMinutes(0);
        }
        ...
        if (!energyByInterval[intervalKeyString]) {
            energyByInterval[intervalKeyString] = { energy: 0, deviceName: reading.deviceName };
        }
